...
static void show_menu(void)
{
    ...
    printf("18. Find age range with B-Tree\n");
    printf("19. Find salary range with B-Tree\n");
    printf("20. Print B-Tree structure\n");
    printf("0. Exit\n");
}
...
int main(void)
{
    ...
    BTreeIndex *age_index = NULL;
    BTreeIndex *salary_index = NULL;

    printf("Creating B-tree indexes...\n");
    age_index = btree_create(5, 0);
    if (!age_index)
    {
        printf("Warning: Failed to create age B-tree index\n");
    }
    else
    {
        printf("Age B-tree created\n");
    }

    salary_index = btree_create(5, 1);
    if (!salary_index)
    {
        printf("Warning: Failed to create salary B-tree index\n");
    }
    else
    {
        printf("Salary B-tree created\n");
    }

    if (age_index && salary_index && db->count > 0)
    {
        printf("Building B-tree indexes from database...\n");
        if (!btree_build_from_database(age_index, db))
        {
            printf("Warning: Failed to build age B-tree\n");
        }
        if (!btree_build_from_database(salary_index, db))
        {
            printf("Warning: Failed to build salary B-tree\n");
        }
        printf("B-tree indexes created (age and salary)\n");
    }
    else if (age_index && salary_index)
    {
        printf("Database is empty, skipping B-tree build\n");
    }
    ...
    while (running)
    {
        ...
        case 1:
        {
            ...
            else
            {
                ...
                if (age_index)
                {
                    btree_insert(age_index, person.age, &db->records[db->count - 1]);
                }
                if (salary_index)
                {
                    btree_insert(salary_index, (int)person.salary, &db->records[db->count - 1]);
                }
            }
            break;
        }
        ...
        case 11:
        {
            ...
            else
            {
                ...
                if (age_index && salary_index && db->count > 0)
                {
                    btree_build_from_database(age_index, db);
                    btree_build_from_database(salary_index, db);
                    printf("B-tree indexes rebuilt\n");
                }
            }
            database_print_stats(db);
            break;
        ...
        case 18:
        {
            ...
            if (age_index)
            {
                RangeResult result = btree_range_query(age_index, min_age, max_age);
                if (result.count > 0)
                {
                    printf("\nFound %d people in age range %d-%d (B-tree):\n", result.count, min_age, max_age);
                    printf("\n%-6s %-20s %-6s %-12s\n", "ID", "Name", "Age", "Salary");
                    printf("------------------------------------------------------\n");
                    for (int i = 0; i < result.count; i++)
                    {
                        printf("%-6d %-20s %-6d %-12.2f\n",
                               ...
                               result.results[i]->age, result.results[i]->salary);
                    }
                }
                else
                {
                    printf("No people found in age range %d-%d\n", min_age, max_age);
                }
                range_result_free(&result);
            }
            else
            {
                printf("B-tree age index not available\n");
            }
            break;
        }
        ...
        case 19:
        {
            double min_salary = 0.0, max_salary = 0.0;
            if (!read_salary_range(&min_salary, &max_salary))
            {
                printf("Invalid salary range input\n");
                break;
            ...
            if (salary_index)
            {
                RangeResult result = btree_range_query(salary_index, (int)min_salary, (int)max_salary);
                if (result.count > 0)
                {
                    printf("\nFound %d people in salary range %.2f-%.2f (B-tree):\n", result.count, min_salary, max_salary);
                    printf("\n%-6s %-20s %-6s %-12s\n", "ID", "Name", "Age", "Salary");
                    printf("------------------------------------------------------\n");
                    for (int i = 0; i < result.count; i++)
                    {
                ...
                {
                    printf("No people found in salary range %.2f-%.2f\n", min_salary, max_salary);
                }
                range_result_free(&result);
            }
            else
            {
                printf("B-tree salary index not available\n");
            }
            break;
        }

        case 20:
            btree_print_structure(age_index);
            btree_print_structure(salary_index);
            break;
